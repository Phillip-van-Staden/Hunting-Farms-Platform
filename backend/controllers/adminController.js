...
exports.getAdminStats = async (req, res) => {
  try {
    ...
    const usersResult = await pool.query(
      "SELECT COUNT(*) AS total_users FROM Person"
    );
    const pendingBlogsResult = await pool.query(
      "SELECT COUNT(*) AS pending_blogs FROM Blogs WHERE bStatus = 'Pending'"
    );
...
exports.getUserReviews = async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `
      SELECT r."rid", r."rstar", r."rdate", r."rdescription",r."rdeleted", f."fname", p."pemail", p."pnaam" || ' ' || p."pvan" AS user
      FROM Review AS r
